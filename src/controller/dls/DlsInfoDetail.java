...
public class DlsInfoDetail extends Controller {
	...
	public void getValueFromPlant() {
		...
		for (Record re : listR) {
			...
			String getInfo = getUserLevel(userId, personlevel, countrylevel, citylevel);
			if (getInfo != null) {
				// 根据getInfo 得到扫码值
	...
	public void getFindUnderScanFromDifferentPlant() {
		...
		List<Record> list = Db
				.find(DlsInfoDetailModel.dao.getFindUnderScanFromDifferentPlantQueryIncome(userId, now).toString());
		List<Map<String, Object>> datalist = new ArrayList<Map<String, Object>>();
		Map<String, Object> map = null;
		for (Record re : list) {
			map = new HashMap<String, Object>();
	 ...
	 */
	@Clear
	private List<Map<Object, Object>> getUnderUserData(List<Map<String, Object>> monthlist,
	 ...
	 */
	@Clear
	private List<Map<String, Object>> getValueFromDifferentPlant(List<Map<String, Object>> list) {
		List<Map<String, Object>> datalist = new ArrayList<Map<String, Object>>();
	 ...
	 */
	/**
	 * <li><div style="color: #FFFFFF; float: left; width: 45%; "> <span style=
	 * "float: left;"> 平台 </span> <span class="pingt"> <span style=
	 * "color: #172E7B; margin-right: 40px;"> 测试平台测试 </span> </span> </div>
	 * <div style=" color: #FFFFFF;float: left; width: 25%; "> <span style=
	 * "float: left; margin-left: 40px;"> 本月收入： </span>
	 * <span class="pingt"> <span style="color: #172E7B; margin-right: 40px;"
	 * > 2633元 </span> </span> </div> <div style=
	 * "color: #FFFFFF; float: left; width: 25%; "> <span style=
	 * "float: left; margin-left: 40px;"> 本月核算： </span> <span style=
	 * "float:right;color: #172E7B; "> 2633元 </span> </div></li>
	 * 
	 * @param list
	 * @return
	 */
	private String jointTable(List<Map<Object, Object>> list) {
		StringBuffer sbuf = new StringBuffer();
		// sbuf.append("<tr
		// class=\"text-center\"><td>平台</td><td>昨日收入</td><td>本月核算</td></tr>");
		if (list.size() > 0) {
			for (Map<Object, Object> map : list) {
				// sbuf.append("<tr class=\"text-center\"><td>" + map.get("pt")
				// + "</td><td>" + isNull(map.get("yestar")) + "</td><td>"
				// + isNull(map.get("mounth")) + "</td></tr>");
				sbuf.append("<li>" + "<div style='color: #FFFFFF; float: left; width: 45%; '>"
						+ "<span style='float: left;'>" + "平台" + "</span>" + "<span class='pingt'>"
						+ "<span style='color: #172E7B; margin-right: 40px;'>" + map.get("pt") + "</span>" + "</span>"
						+ "</div>" + "<div style=' color: #FFFFFF;float: left; width: 25%; '>"
						+ "<span style='float: left; margin-left: 40px;'>" + "昨日收入：" + "</span>"
						+ "<span class='pingt'>" + "<span style='color: #172E7B; margin-right: 40px;'>"
						+ isNull(map.get("yestar")) + "</span>" + "</span>" + "</div>"
						+ "<div style='color: #FFFFFF; float: left; width: 25%; '>"
						+ "<span style='float: left; margin-left: 40px;'>" + "本月核算：" + "</span>"
						+ "<span style='float:right;color: #172E7B; '>" + isNull(map.get("mounth")) + "</span>"
						+ "</div>" + "</li>");
			}
		}
	...
	private String jointScanTable(List<Map<String, String>> list) {
		StringBuffer sbuf = new StringBuffer();
		// sbuf.append("<tr
		// class=\"text-center\"><td>平台</td><td>扫码时间</td><td>取关时间</td><td>扫码昵称</td><td>收入</td></tr>");
		if (list.size() > 0) {
			for (Map<String, String> map : list) {
				...
				sbuf.append("<li>" + "<div style='text-align:left; width:34%;'>" + "<img src='" + map.get("headimgurl")
						+ "' class='img-circle' style='float:left; margin-top:13px; margin-left:30px; width:35px;'>"
						+ "<span style='font-size: 17px; margin-left: 30px;  width:70%; white-space:nowrap;overflow:hidden; text-overflow:ellipsis; float:left;'>"
						+ map.get("attpeople") + "</span>" + "</div>" + "<div style=' width:12%; '>"
						+ "<span style='color: red; font-size: 17px;'>" + map.get("value") + "</span>"
						+ "<span style='color: red; font-size: 17px;'>" + "元" + "</span>" + "</div>"
						+ "<div style=' width:26%; white-space:nowrap; text-overflow:ellipsis; overflow:hidden;'>"
						+ "<span style=' width:120px; font-size: 17px;  '>" + map.get("attentionPL") + "</span>"
						+ "</div>" + "<div style=' width:26%; '>" + "<span style='font-size: 17px; '>"
						+ isNull(map.get("time")) + "</span>" +
	...
	public void getScanFlowInfo() {
		...
		for (Record re : list) {
			...
			if (object != null) {
				...
				map.put("time", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(re.get("smtime")));
			}
	...
	public void getScanFlowInfoForWap() {
		...
		for (Record re : list) {
			 ...
			 * if (re.get("cancletime") != null) {
			 * 
			 * map.put("value", "- " + re.get(getInfo).toString()); } else {
			 * map.put("value", re.get(getInfo).toString()); }
			 */
			...
			if (object != null) {
				...
				map.put("smtime", new SimpleDateFormat("yy-MM-dd HH:mm").format(re.get("smtime")));
			}
			// map.put("cancletime", timeIsNull(re.get("cancletime")));
			map.put("attpeople", re.get("attpeople").toString());
			map.put("headimgurl", re.getStr("headimgurl"));
	...
	private String getUserLevel(int userId, Object personlevel, Object countrylevel, Object citylevel) {
		if (personlevel != null && userId == Integer.valueOf(personlevel.toString())) {
			return "personget";
		} else if (countrylevel != null && userId == Integer.valueOf(countrylevel.toString())) {
			return "countryget";
		} else if (citylevel != null && userId == Integer.valueOf(citylevel.toString())) {
			return "cityget";
		} else {
			return null;
		}
	...
	public void getYestardayIncomeForDifferentAgent() {
		...
		int userId = getParaToInt("userId");
		String inCome = doCalculateIncomeForDifferentAgent("2", angentType, loginUser, userId);
	...
	public void getIncomeForDifferentAgent() {
		...
		String inCome = doCalculateIncomeForDifferentAgent("1", angentType, loginUser, userId);
		final JSONObject result = new JSONObject();
		result.put("inCome", inCome);
		final JSONObject retObj = (JSONObject) JSONObject.toJSON(result);
		renderJson(retObj);
	...
	public void getIncomeForDifferentAgent2() {
		...
		String inCome = doCalculateIncomeForDifferentAgent("4", angentType, loginUser, userId);
		final JSONObject result = new JSONObject();
		result.put("inCome", inCome);
		final JSONObject retObj = (JSONObject) JSONObject.toJSON(result);
		renderJson(retObj);
	...
	public void getBalanceForDifferentAgent() {
		...
		String loginUser = getPara("userName");
		int userId = getParaToInt("userId");
		String inCome = doCalculateIncomeForDifferentAgent("3", angentType, loginUser, userId);
		final JSONObject result = new JSONObject();
		result.put("inCome", inCome);
		final JSONObject retObj = (JSONObject) JSONObject.toJSON(result);
		renderJson(retObj);
	...
	private String doCalculateIncomeForDifferentAgent(String day, int angentType, String loginUser, int userId) {
		DecimalFormat format = new DecimalFormat("0.00");
		String result = format.format(DlsInfoDetailModel.dao.doCalculateIncomeForDifferentAgentExtractionTime(day,
				angentType, loginUser, userId));
		return result;
	...
	public void moban() {
		...
		// List<Record> list =
		// Db.find(DlsInfoDetailModel.dao.moban(userId).toString());
		List<Record> list = Db.find(DlsInfoDetailModel.dao.getScanFlowInfosaomiao(agentType, userId).toString());
		...
		for (Record re : list) {
			...
			Object citylevel = re.get("citylevel");
			String getInfo = getUserLevel(userId, personlevel, countrylevel, citylevel);
			String inCome = doCalculateIncomeForDifferentAgent("1", agentType, loginname, userId);
			String nowCome = doCalculateIncomeForDifferentAgent("0", agentType, loginname, userId);
			...
			Object object = re.get("cancletime");
			if (object != null) {
				map.put("smtime", timeIsNull(re.get("cancletime")));
			} else {
				map.put("smtime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(re.get("smtime")));
			}

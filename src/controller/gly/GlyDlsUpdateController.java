 ...
 *
 */
public class GlyDlsUpdateController extends Controller {
	...
	public void tixiantimeone(){
		render("/guanliyuan/one.html");
	}
	public void tixiantimetwo(){
		String tixiantype = getPara("tixiantype");
		String tixianmoney = getPara("tixianmoney");
		TixianTime ti=new TixianTime();
		ti.set("id", 202);
		ti.set("tixiantype", tixiantype);
		ti.set("tixianmoney",tixianmoney);
		TixianTime.dao.saveTixianTime(ti);
		int n=1;
		renderJson(n);
	}
	public void tixiantimethree(){
		TixianTime findTixianTime = TixianTime.dao.findTixianTime();
		String tixiantype = findTixianTime.get("tixiantype");
		if(tixiantype.equals("1")){
			renderJson(1);
		}else if(tixiantype.equals("2")){
			String tixianmoney= findTixianTime.get("tixianmoney");
			renderJson(tixianmoney);
		}else{
			int sessionAttr = getSessionAttr("currentUserId");
			TixianUserTime findUserTime =
					TixianUserTime.dao.findUserTime(sessionAttr);
			String userTime= findUserTime.get("usertime").toString();
			System.out.println(userTime);
			String meishi1= meishi(userTime);
			String time = GetTime.getTime();
			String meishi2 = meishi(time);
			if(meishi1.equals(meishi2)){
				renderJson(1);
			}else{
				renderJson(2);
			}
	...
	public void tixiantimefour(){
		int sessionAttr = getSessionAttr("currentUserId");
		TixianUserTime findUserTime =
				TixianUserTime.dao.findUserTime(sessionAttr);
		String ss = findUserTime.get("usertime").toString();
		String meishi = meishi(ss);
		int parseInt = Integer.parseInt(meishi);
		System.out.println(parseInt);
		renderJson(parseInt);
	}
	public void tixiantimefive(){
		TixianTime findTixianTime = TixianTime.dao.findTixianTime();
		String number = findTixianTime.get("tixiantype");
		int n=Integer.parseInt(number);
		renderJson(n);
	}
	public void tixiantimesix(){
		TixianTime findTixianTime = TixianTime.dao.findTixianTime();
		String no = findTixianTime.get("tixianmoney");
		int parseInt = Integer.parseInt(no);
		renderJson(parseInt );
	}
	private String meishi(String time){
